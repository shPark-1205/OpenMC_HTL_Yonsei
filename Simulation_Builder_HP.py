...
class NuclearFusion:
    ...
    def _create_surfaces(self):
        ...
        surfaces['inner_multiplier_radius'] = openmc.XCylinder(r=geo_config['inner_multiplier']['radius'],
                                                   name='Inner Multiplier Radius')  # 핀 내부 증배재의 바깥쪽 면
        ...
        surfaces['impinging_plate'] = openmc.XPlane(x0=self.tokamak_radius
                                        ...
                                        name='Impinging Plate')  # 충돌 면 (FW에서 24 mm 뒤에 위치)
        ...
        surfaces['nozzle_tip_pos'] = openmc.XPlane(x0=self.tokamak_radius
                                          ...
                                          + geo_config['first_wall']['channel_height']
                                          + geo_config['nozzle_to_target']
                                          + geo_config['pin']['width'],
                                       name='Nozzle Tip Positive')  # 핀 노즐의 윗면
        surfaces['inner_multiplier_max_pos'] = openmc.XPlane(x0=self.tokamak_radius
                                                    + geo_config['first_wall']['armor_height']
                                                    + geo_config['first_wall']['channel_height']
                                                    + geo_config['nozzle_to_target']
                                                    + geo_config['pin']['width']
                                                    + geo_config['inner_multiplier']['length'],
                                                 name='Multiplier Max Positive')  # 핀 내부 증배재의 길이 (18 cm)
        ...
        # Outer 증배재 사이의 평면.
        # -half-space가 증배재 안쪽을 보도록 벡터 설정
        # 별도 PPT 파일 육각형 그림 참고
        surfaces['outer_multiplier_1_6'] = openmc.Plane(a=0.0, b=1.0, c=0.0, d=-geo_config['outer_multiplier']['pitch'] / 2.0,
                                            name='Outer Multiplier 1-6')  # -half-space: 증배재 안쪽
        surfaces['outer_multiplier_2_9'] = openmc.Plane(a=0.0, b=-1.0, c=-np.sqrt(3.0), d=-geo_config['outer_multiplier']['pitch'],
                                            name='Outer Multiplier 2-9')  # -half-space: 증배재 안쪽
        surfaces['outer_multiplier_3_8'] = openmc.Plane(a=0.0, b=1.0, c=np.sqrt(3.0), d=-geo_config['outer_multiplier']['pitch'],
                                            name='Outer Multiplier 3-8')  # -half-space: 증배재 안쪽
        surfaces['outer_multiplier_4_11'] = openmc.Plane(a=0.0, b=1.0, c=-np.sqrt(3.0), d=-geo_config['outer_multiplier']['pitch'],
                                             name='Outer Multiplier 4-11')  # -half-space: 증배재 안쪽
        surfaces['outer_multiplier_5_10'] = openmc.Plane(a=0.0, b=-1.0, c=np.sqrt(3.0), d=-geo_config['outer_multiplier']['pitch'],
                                             name='Outer Multiplier 5-10')  # -half-space: 증배재 안쪽
        surfaces['outer_multiplier_7_12'] = openmc.Plane(a=0.0, b=-1.0, c=0.0, d=-geo_config['outer_multiplier']['pitch'] / 2.0,
                                             name='Outer Multiplier 7-12')  # -half-space: 증배재 안쪽
    ...
    def _create_regions(self, surfaces):
        ...
        # First wall channel
        regions['first_wall_channel_region'] = periodic_boundary & +surfaces['first_wall_channel_side'] & -surfaces['impinging_plate']

        # Pressure tube
        regions['tube_region'] = -surfaces['tube_outer_radius'] & +surfaces['tube_inner_radius'] & +surfaces['impinging_plate'] & -surfaces['bound_x_max']

        # 증식재/증배재 담는 pin
        pin_region_outer = -surfaces['outer_pin_outer_radius'] & +surfaces['outer_pin_inner_radius'] & +surfaces['nozzle_tip_neg'] & -surfaces['bound_x_max']
        pin_region_inner = -surfaces['inner_pin_outer_radius'] & +surfaces['inner_pin_inner_radius'] & +surfaces['nozzle_tip_neg'] & -surfaces['bound_x_max']
        pin_region_nozzle = -surfaces['outer_pin_inner_radius'] & +surfaces['inner_pin_outer_radius'] & +surfaces['nozzle_tip_neg'] & -surfaces['nozzle_tip_pos']
        regions['pin_region'] = pin_region_outer | pin_region_inner | pin_region_nozzle

        # Pin 내부 증배재
        regions['inner_multiplier_region'] = -surfaces['inner_multiplier_radius'] & +surfaces['inner_pin_outer_radius'] & +surfaces['nozzle_tip_pos'] & -surfaces['inner_multiplier_max_pos']

        # Pin 내부 증식재
        breeder_region_outer = -surfaces['outer_pin_inner_radius'] & +surfaces['inner_multiplier_radius'] & +surfaces['nozzle_tip_pos'] & -surfaces['bound_x_max']
        breeder_region_inner = -surfaces['inner_multiplier_radius'] & +surfaces['inner_pin_outer_radius'] & +surfaces['inner_multiplier_max_pos'] & -surfaces['bound_x_max']
        regions['breeder_region'] = breeder_region_outer | breeder_region_inner

        # Pin 외부 증배재
        outer_multiplier_pieces = [
            +surfaces['outer_multiplier_inner_radius'] & -surfaces['outer_multiplier_1_6'] & -surfaces[
                'outer_multiplier_2_9'] & +surfaces['periodic_z_pos'] & -surfaces['periodic_y_neg_z_pos'],
            +surfaces['outer_multiplier_inner_radius'] & -surfaces['outer_multiplier_3_8'] & -surfaces[
                'outer_multiplier_4_11'] & -surfaces['periodic_y_neg_z_pos'] & +surfaces['periodic_y_neg_z_neg'],
            +surfaces['outer_multiplier_inner_radius'] & -surfaces['outer_multiplier_5_10'] & -surfaces[
                'outer_multiplier_1_6'] & +surfaces['periodic_y_neg_z_neg'] & -surfaces['periodic_z_neg'],
            +surfaces['outer_multiplier_inner_radius'] & -surfaces['outer_multiplier_7_12'] & -surfaces[
                'outer_multiplier_3_8'] & -surfaces['periodic_z_neg'] & +surfaces['periodic_y_pos_z_neg'],
            +surfaces['outer_multiplier_inner_radius'] & -surfaces['outer_multiplier_2_9'] & -surfaces[
                'outer_multiplier_5_10'] & +surfaces['periodic_y_pos_z_neg'] & -surfaces['periodic_y_pos_z_pos'],
            +surfaces['outer_multiplier_inner_radius'] & -surfaces['outer_multiplier_4_11'] & -surfaces[
                'outer_multiplier_7_12'] & -surfaces['periodic_y_pos_z_pos'] & +surfaces['periodic_z_pos'],
        ]

        # 모든 조각들을 하나로 합치고, 공통적인 축 방향 길이를 한 번에 적용
        regions['outer_multiplier_region'] = (openmc.Union(outer_multiplier_pieces)
                                              & +surfaces['outer_multiplier_min_pos']
                                              & -surfaces['outer_multiplier_max_pos'])

        # Pin 내부 헬륨
        tube_region_inner = -surfaces['tube_outer_radius'] & +surfaces['impinging_plate'] & -surfaces['bound_x_max']
        regions['inner_he_region'] = (tube_region_inner
                                      & ~(regions['tube_region'] | regions['pin_region'] | regions['inner_multiplier_region'] | regions['breeder_region'])
                                      )

        # Pin 외부 헬륨
        tube_region_outer = periodic_boundary & +surfaces['impinging_plate'] & -surfaces['outer_multiplier_max_pos'] & +surfaces['tube_outer_radius']
        regions['outer_he_region'] = tube_region_outer & ~regions['outer_multiplier_region']
        ...
        filled_material_regions = [
            ...
            regions['tube_region'],
            regions['pin_region'],
            regions['inner_multiplier_region'],
    ...
    def _create_cells(self, regions):
        ...
        cells['pin_cell'] = openmc.Cell(fill=self.materials['eurofer'],
                                        ...
                                        name='Pin')
        cells['inner_multiplier_cell'] = openmc.Cell(fill=self.materials['Be12Ti'],
                                                       region=regions['inner_multiplier_region'], cell_id=5,
        ...
        self.inner_multiplier_cell = cells['inner_multiplier_cell']
        self.breeder_cell = cells['breeder_cell']
        self.outer_multiplier_cell = cells['outer_multiplier_cell']
        self.helium_cell_inner = cells['helium_cell_inner']
        self.helium_cell_outer = cells['helium_cell_outer']
    ...
    def generate_geometry_2D_plots(self, plots_folder='plots'):
        ...
        try:
            ...
            fig_xy.savefig(os.path.join(plots_folder, 'geometry_2D_xy_HP.png'), dpi=600)  # 파일 저장 및 이동
            plt.close(fig_xy)
            ...
            ax_yz1 = self.geometry.plot(
                ...
                origin=(self.tokamak_radius
                        ...
                        + self.config['geometry']['pin']['width']
                        + (self.config['geometry']['inner_multiplier']['length']*0.5), 0, 0),  # 그림의 원점
            ...
            fig_yz1.savefig(os.path.join(plots_folder, 'geometry_2D_yz_multiplier_HP.png'), dpi=600)  # 파일 저장 및 이동
            plt.close(fig_yz1)
            print("YZ-1 geometry plot with axes saved.\n")
            ...
            fig_yz2.savefig(os.path.join(plots_folder, 'geometry_2D_yz_breeder_HP.png'), dpi=600)  # 파일 저장 및 이동
            plt.close(fig_yz2)
    ...
    def preview_source_distribution(self, plots_folder='plots', n_samples=1000):
        try:
            ...
            plot_pos.write_html(os.path.join(plots_folder, 'source_preview_position_HP.html'))
            ...
            plot_en.write_html(os.path.join(plots_folder, 'source_preview_energy_HP.html'))
            ...
            plot_dir.write_html(os.path.join(plots_folder, 'source_preview_direction_HP.html'))
    ...
    def define_tallies(self):
        ...
        try:
            ...
            tally_multiplying.filters = [inner_multiplier_cell_filter, outer_multiplier_cell_filter]
            ...
            mesh_y_min = -mesh_config['length_y'] / 2.0
            ...
            tally_local_heating_inner_multiplier_neutron = openmc.Tally(name='local_heating_inner_multiplier_neutron')
            tally_local_heating_inner_multiplier_photon = openmc.Tally(name='local_heating_inner_multiplier_photon')
            ...
            tally_local_heating_inner_multiplier_neutron.scores = ['heating']
            tally_local_heating_inner_multiplier_photon.scores = ['heating']
            ...
            tally_local_heating_inner_multiplier_neutron.filters = [mesh_filter, inner_multiplier_cell_filter,openmc.ParticleFilter(['neutron'])]
            tally_local_heating_inner_multiplier_photon.filters = [mesh_filter, inner_multiplier_cell_filter,openmc.ParticleFilter(['photon'])]
            ...
            local_tallies_list = [
                ...
                tally_local_heating_inner_multiplier_neutron,
                tally_local_heating_inner_multiplier_photon,
